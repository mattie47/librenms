...
// For Reference:
//      https://github.com/librenms/librenms/blob/master/mibs/awplus/AT-ATMF-MIB
//      https://www.alliedtelesis.com/documents/network-time-protocol-ntp-feature-overview-and-configuration-guide
$atAtmfSummary = snmpwalk_group($device, 'atAtmfSummary', 'AT-ATMF-MIB');

print_r($atAtmfSummary);
/*
...
//    foreach ($atAtmfSummary as $index => $value) {
        $result = array();
...
if (count($components) > 0) {
    if (dbFetchCell('SELECT COUNT(*) FROM `applications` WHERE `device_id` = ? AND `app_type` = ?', array($device['device_id'], $module)) == '0') {
        dbInsert(array('device_id' => $device['device_id'], 'app_type' => $module, 'app_status' =>  $result['status'], 'app_instance' => ''), 'applications');
    }
} else {
